...
#ifndef __CURE_LIST_H__
#define __CURE_LIST_H__
 ...
 *
 * zQuickSort() sorts an array pointed by \a array with
 * quick sort method. \a size is the number of factors
 * of \a array.
 * The factors of \a array will be sorted in ascending order
 * according to the comparison function \a cmp.
 * Namely, a factor 'a' in the \a array is put after another
 * factor 'b' if cmp(a,b,p) > 0, where \a p is for programmer's
 * utility, given by \a priv.
 * \return
 * a pointer \a array is returned.
 */
__EXPORT void *zQuickSort(void *array[], int size, int (*cmp)(void*,void*,void*), void *priv);

__END_DECLS

#endif /* __CURE_LIST_H__ */
